...
Those files, now in a local folder, contain all the inventory data for all provinces. We preprocess all available inventory data, but for our analysis to be able to proceed we need to choose only the IFN2 and IFN3 inventories. Package $\texttt{forestables}$ provides tools to select those datasets.
...
plan("multisession", workers = 6)
ifn4 <- ifn_to_tibble(provinces = provinces,
                      versions = c("ifn4"),
                      folder = ifn_path)
...
ifn4 <- ifn4 |> clean_empty("tree")
```


```{r include = FALSE}
# Uncomment if run first time.
# save(ifn2, ifn3, ifn4, file = ".\\data-raw\\ifn_2_3_4.RData")
load("..\\data-raw\\ifn_2_3_4.RData")
```
