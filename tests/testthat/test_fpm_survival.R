test_that("Sapling model", {
  ...
  df <- as.data.frame(survival_model$`Pinus sylvestris`$model)
  df$alive <- NULL
  ...
  # Bigger arrays.
  # a2 <- rbind(a, a)
  # for (i in 1:5) a2 <- rbind(a2, a2)
  # df2 <- rbind(df, df)
  # for (i in 1:5) df2 <- rbind(df2, df2)
  ...
  models_list <- list(survival_model = survival_model)

  b <- fpm_survival(a, df, models_list, verbose = T)
