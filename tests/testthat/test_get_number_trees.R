test_that("Testing number of trees extraction", {
  ...
  # Same length test.
  N_a <- get_number_trees(a)
  N_b <- get_number_trees(b)

  expect_length(N_a, nrow(a))
  expect_length(N_a, nrow(b))

  expect_equal(N_a, N_b)

  # Sum of ba for species test.
  N_a_sp <- get_number_trees(a, per_species = T)
  N_b_sp <- get_number_trees(b, per_species = T)

  expect_true(all(sapply(1:length(N_a_sp), function(i) {
    x <- N_a_sp[[i]]
    y <- N_b_sp[[i]]
    all.equal(unlist(x[names(y)]), unlist(y))
  })))
